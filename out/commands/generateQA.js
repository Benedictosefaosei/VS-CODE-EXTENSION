...
async function generateQA() {
    ...
    if (!fs.existsSync(configPath)) {
        ...
        try {
            ...
            vscode.window.showInformationMessage("Config file 'cqlc.config.json' was not found. A default config file has been created. Please modify it with your specific paths and settings, then run the generate command again.", { modal: true });
    ...
    if (missingFields.length > 0) {
        ...
        const response = await vscode.window.showErrorMessage(`Config file is missing required fields: ${missingFields.join(', ')}. Please update the config file and try again.`, openConfig);
...
//# sourceMappingURL=generateQA.js.map